...
from tri_api.models.tenant.tenant import ActiveTokenList
from fastapi import Query, APIRouter, Response, HTTPException
from tri_api.celery.tasks import populate_trove_intel_database
...
async def get_tasks_list(
    ...
    total_tasks = await ScannerTask.find().count()

    tasks = await ScannerTask.find().skip(skip).limit(page_size).to_list()
