...
@router.post(
    ScannerRouteEnum.software_bill_of_materials.value,
)
async def create_software_bill_of_materials_task(
    ...
    scanner_task = ScannerTask(
        ...
        scanner=ScannerType.software_bill_of_materials.value,
    )
...
async def get_software_bill_of_materials_tasks(
    user: User = Depends(current_user),
    ...
    total_tasks = await ScannerTask.find(
        ...
        ScannerTask.scanner == ScannerType.software_bill_of_materials.value,
    ).count()
