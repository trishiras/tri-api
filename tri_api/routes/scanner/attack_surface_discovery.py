...
@router.post(
    ScannerRouteEnum.attack_surface_discovery.value,
)
async def create_attack_surface_discovery_task(
    ...
    scanner_task = ScannerTask(
        ...
        scanner=ScannerType.attack_surface_discovery.value,
    )
...
async def get_attack_surface_discovery_tasks(
    user: User = Depends(current_user),
    ...
    total_tasks = await ScannerTask.find(
        ...
        ScannerTask.scanner == ScannerType.attack_surface_discovery.value,
    ).count()
