...
@router.post(
    ScannerRouteEnum.attack_surface_analysis.value,
)
async def create_attack_surface_analysis_task(
    arguments: ScannerTaskArguments,
    user: User = Depends(current_user),
):
    if arguments.target_type != TargetType.domain.value:
        raise HTTPException(
    ...
    scanner_task = ScannerTask(
        ...
        scanner=ScannerType.attack_surface_analysis.value,
    )
...
async def get_attack_surface_analysis_tasks(
    user: User = Depends(current_user),
    ...
    total_tasks = await ScannerTask.find(
        ...
        ScannerTask.scanner == ScannerType.attack_surface_analysis.value,
    ).count()
