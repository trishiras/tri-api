...
@router.post(
    ScannerRouteEnum.cloud_security_posture_management.value,
)
async def create_cloud_security_posture_management_task(
    arguments: ScannerTaskArguments,
    user: User = Depends(current_user),
):
    if arguments.target_type != TargetType.cloud.value:
        raise HTTPException(
            status_code=ResponseCode.bad_request.value,
            detail=ScannerExceptionMessage.incorrect_input.value,
        )

    if arguments.target.upper() != "AWS":
        raise HTTPException(
            status_code=ResponseCode.bad_request.value,
            detail=ScannerExceptionMessage.incorrect_input.value,
        )
    ...
    scanner_task = ScannerTask(
        ...
        scanner=ScannerType.cloud_security_posture_management.value,
    )
...
async def get_cloud_security_posture_management_tasks(
    user: User = Depends(current_user),
    ...
    total_tasks = await ScannerTask.find(
        ...
        ScannerTask.scanner == ScannerType.cloud_security_posture_management.value,
    ).count()
