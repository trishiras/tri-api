...
@router.post(
    ScannerRouteEnum.dynamic_application_security_testing.value,
)
async def create_dynamic_application_security_testing_task(
    arguments: ScannerTaskArguments,
    user: User = Depends(current_user),
):
    if arguments.target_type != TargetType.url.value:
        raise HTTPException(
    ...
    scanner_task = ScannerTask(
        ...
        scanner=ScannerType.dynamic_application_security_testing.value,
    )
...
async def get_dynamic_application_security_testing_tasks(
    user: User = Depends(current_user),
    ...
    total_tasks = await ScannerTask.find(
        ...
        ScannerTask.scanner == ScannerType.dynamic_application_security_testing.value,
    ).count()
    ...
    tasks = (
        await ScannerTask.find(
            ScannerTask.user == user.id,
            ScannerTask.scanner
            == ScannerType.dynamic_application_security_testing.value,
        )
        .skip(skip)
        .limit(page_size)
        .to_list()
