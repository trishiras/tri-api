...
@router.post(
    ScannerRouteEnum.static_application_security_testing.value,
)
async def create_static_application_security_testing_task(
    ...
    scanner_task = ScannerTask(
        ...
        scanner=ScannerType.static_application_security_testing.value,
    )
...
async def get_static_application_security_testing_tasks(
    user: User = Depends(current_user),
    ...
    total_tasks = await ScannerTask.find(
        ...
        ScannerTask.scanner == ScannerType.static_application_security_testing.value,
    ).count()
    ...
    tasks = (
        await ScannerTask.find(
            ScannerTask.user == user.id,
            ScannerTask.scanner
            == ScannerType.static_application_security_testing.value,
        )
        .skip(skip)
        .limit(page_size)
        .to_list()
