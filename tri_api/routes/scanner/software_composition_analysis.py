...
@router.post(
    ScannerRouteEnum.software_composition_analysis.value,
)
async def create_software_composition_analysis_task(
    arguments: ScannerTaskArguments,
    user: User = Depends(current_user),
):
    if arguments.target_type not in [
        TargetType.repository.value,
        TargetType.container_image.value,
    ]:
        raise HTTPException(
    ...
    scanner_task = ScannerTask(
        ...
        scanner=ScannerType.software_composition_analysis.value,
    )
...
async def get_software_composition_analysis_tasks(
    user: User = Depends(current_user),
    ...
    total_tasks = await ScannerTask.find(
        ...
        ScannerTask.scanner == ScannerType.software_composition_analysis.value,
    ).count()
